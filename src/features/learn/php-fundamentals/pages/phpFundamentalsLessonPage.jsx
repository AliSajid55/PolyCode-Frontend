...
import PhpFundamentalsCodeChallenge from "../components/PhpFundamentalsCodeChallenge";
import {
...
const READ_GATE_PREFIX = "php_fundamentals";
...
export default function PhpFundamentalsLessonPage() {
  ...
  const lessonIdx = PHP_FUNDAMENTALS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = PHP_FUNDAMENTALS_LESSONS[lessonIdx - 1];
  const next = PHP_FUNDAMENTALS_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>PHP lesson not found.</p>
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to PHP Fundamentals
        </button>
      </div>
    );
  }
  ...
  const earnedXP = PHP_FUNDAMENTALS_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="PHP Fundamentals"
            syncLabel={
              isAuthenticated
                ? "PHP progress saved to your account"
        ...
        <LessonContentShell
          storageKey={`php-fundamentals:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — PHP`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
