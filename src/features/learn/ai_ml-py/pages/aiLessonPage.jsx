...
const READ_GATE_PREFIX = "ai_ml_py";
...
export default function AiLessonPage() {
  ...
  const lessonIdx = AI_LESSONS.findIndex((item) => item.id === lessonId);
  const prev      = AI_LESSONS[lessonIdx - 1];
  const next      = AI_LESSONS[lessonIdx + 1];
  ...
  const earnedXP = AI_LESSONS.filter((item) => progress[item.id]).reduce(
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            ...
            syncLabel={
              isAuthenticated
                ? "AI/ML progress saved to your account"
        ...
        <LessonContentShell
          storageKey={`aiml-py:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — AI/ML`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
